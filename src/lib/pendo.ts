...
// Session-scoped key — same key used by src/lib/visitor.ts
const VISITOR_STORAGE_KEY = "fc-session-visitor-id";

/**
 * Inline install in document <head>: stub loads pendo.js, queues initialize,
 * enables session replay. Visitor ID is session-scoped (sessionStorage) so
 * Pendo and the DB use the exact same anonymous ID per session.
 * SPA pageLoad is handled only by PendoPageTracker.
 */
...
var visitorId=sessionStorage.getItem('${VISITOR_STORAGE_KEY}');
if(!visitorId){visitorId='anon-'+crypto.randomUUID();sessionStorage.setItem('${VISITOR_STORAGE_KEY}',visitorId);}
