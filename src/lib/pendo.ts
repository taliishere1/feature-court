...
pendo.initialize({visitor:{id:visitorId},account:{id:'feature-court1'},recording:{enabled:true,autoStart:true},sanitizeUrl:function(u){try{var x=new URL(u);var p=x.pathname;if(p.indexOf('/verdict/')===0)p='/verdict';return x.origin+p}catch(e){return u.split('?')[0].split('#')[0];}}});
})('${PENDO_API_KEY}');`;
