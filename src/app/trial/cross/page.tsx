...
import { CourtSeal, StageProgress, OrnateDivider } from "@/components/court-components";
      ...
      <main className="flex-1 px-6 py-12 relative z-10">
        <div className="max-w-3xl mx-auto animate-page-enter">
          ...
          <div className="text-center mb-10">
            <div className="parchment p-6 animate-fade-in-up inline-block">
              ...
              <p className="text-court-300 text-sm italic font-legal leading-relaxed">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {trial.cross_examination.map((question, i) => (
              <div key={i} className="parchment p-5 animate-fade-in-up" style={{ animationDelay: `${0.25 + i * 0.15}s` }}>
                <div className="flex items-center gap-2 mb-3">
                  <span className="font-mono text-[10px] text-gold-500">Question {i + 1}</span>
                </div>
                <label className="block text-court-200 font-serif text-base mb-3 leading-relaxed">
                  {question}
                </label>
                <textarea
                  required
                  ...
                  className="w-full bg-transparent border border-court-700 rounded-sm px-4 py-3 text-court-100 placeholder:text-court-600/40 focus:outline-none focus:border-gold-500/60 transition-colors resize-none text-sm font-legal tracking-wide"
                />
              </div>
            ))}
